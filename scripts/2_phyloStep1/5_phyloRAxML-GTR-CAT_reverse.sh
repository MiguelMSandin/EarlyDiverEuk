...
FASTA="pb_pr2_reads10c_r2_align_trim05.fasta"
...
for SLURM_ARRAY_TASK_ID in $(seq 10); do
	OUTPUT=${FASTA/.fasta/_raxml-GTRcat-D$SLURM_ARRAY_TASK_ID}
	raxmlHPC-PTHREADS-AVX2 -g $CONSTRAINT -T $THREADS -m GTRCAT -p $RANDOM -D -N $BS -n $OUTPUT -s $FILE --no-seq-check
done

# Now search for the best scoring tree and annotate the 100 total bootstrap in the tree ------------
BS_ALL="RAxML_bootstrap.${FASTA/.fasta/_raxml-GTRcat.tre}"
OUT=${FASTA/.fasta/_raxml-GTRcat}
