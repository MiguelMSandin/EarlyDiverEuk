...
FILE="pb_pr2_reads10c_align_trim05.fasta"
...
for SLURM_ARRAY_TASK_ID in $(seq 10); do
	PREFIX="step1f_RAng$SLURM_ARRAY_TASK_ID"
...
grep "Final LogLikelihood" *log > "likelihoods.tsv"
sed -i "s/:Final LogLikelihood: /\t/g" "likelihoods.tsv"

for LL in $(cut -f 2 "likelihoods.tsv" | sort | head -n 4); do
	LLL=${LL/-/}
	BEST_TREE=$(grep "$LLL" "likelihoods.tsv" | cut -f 1)
	echo "$BEST_TREE"
	echo "$BEST_TREE" >> "best_trees.list"
done
