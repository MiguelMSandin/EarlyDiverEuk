...
outTipRates = paste0(prefix, "_tipRates.txt")
outLTTparam = paste0(prefix, "_parameters.txt")
outTree = paste0(prefix, "_RTTtree.pdf")
outRTTplot = paste0(prefix, "_RTT.pdf")
outRTTtable = paste0(prefix, "_RTT.tsv")
outDTTplot = paste0(prefix, "_DTT.pdf")
outDTTtable = paste0(prefix, "_DTT.tsv")
...
root = max(branching.times(CladsOutput$tree))

# Extracting tip rates _____________________________________________________________________________
if(args$verbose){cat("  Extracting tip rates\n")}
write.table(CladsOutput$lambdatip_map, outTipRates, quote=FALSE, sep="\t", row.names=FALSE, col.names=FALSE)

# Extracting model parameters ______________________________________________________________________
if(args$verbose){cat("  Extracting model parameters\n")}
param = data.frame(file=sub("\\.Rdata", "", args$file), sigma=CladsOutput$sig_map, alpha=CladsOutput$al_map, epsilon=CladsOutput$eps_map, lambda0=CladsOutput$lambda0_map, meanLambdaTip=mean(CladsOutput$lambdatip_map))
write.table(param, outLTTparam, quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
...
plot_ClaDS_phylo(tree, rates, show.tip.label=TRUE, cex=0.2, log=TRUE)
title(prefix)
tmp = dev.off()
...
clads_rtt$time[1] = ifelse(clads_rtt$time[1] < -root, -root, clads_rtt$time[1])
...
if(args$verbose){cat("  Plotting diversity Through Time plot\n")}

clads_dtt = data.frame(time_points=CladsOutput$time_points,
                       time=-rev(CladsOutput$time_points),
                       lineages=CladsOutput$DTT_mean)

# enhanced = data.frame(time=-rev(CladsOutput$time_points))
enhanced = data.frame(time=clads_dtt$time)
for(t in 1:length(CladsOutput$enhanced_trees)){
	...
	for(ti in enhanced$time){
	  i = ifelse(ti <= -root, 2, max(subset(tmp, time < ti)$N))
		tmp1 = c(tmp1, i)
	}
...
clads_dtt$mean = apply(enhanced[,-1], 1, mean)
clads_dtt$min = apply(enhanced[,-1], 1, min)
clads_dtt$max = apply(enhanced[,-1], 1, max)

clads_dtt$time[1] = ifelse(clads_dtt$time[1] < -root, -root, clads_dtt$time[1])

geos = subset(geo, time>min(clads_dtt$time))
geos$mid = apply(data.frame(geos$time, c(geos$time[-1], 0)), 1, mean)

lttPlot =ggplot(clads_dtt, aes(x=time, y=lineages))+
		geom_vline(xintercept = geos$time, color="lightgrey") +
		# geom_point()+
		...
		scale_y_log10() + annotation_logticks(sides = 'l')+
		scale_x_continuous(breaks=seq((round(min(clads_dtt$time), -2)), 0, 100),
						   minor_breaks=seq((round(min(clads_dtt$time), -2)), 0, 50)) +
		theme_classic()+
		labs(title=paste0(prefix),
			 y="Diversity Through Time (Ln)", x="Time (Ma)")

pdf(outDTTplot, width=11.69, height=8.27, paper='special')
plot(lttPlot)
tmp = dev.off()
write.table(clads_dtt, outDTTtable, quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)
