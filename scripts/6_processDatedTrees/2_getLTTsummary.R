...
files = list(dirs=c("stepDate/rootD"), # The directory(ies) where the dated trees are
             ...
             out="output/LTT/rootDiscoba_cleaned_LTT") # The output files without extension for the tsv table (*.tsv) and the pdf (*.pdf)

rm(list=ls()[!ls() %in% c("geo", "files")])
if(!dir.exists("output")){dir.create("output")}
if(!dir.exists("output/LTT")){dir.create("output/LTT")}
...
files$files = c()
for(d in files$dirs){
	tmp = paste0(d, "/", grep(paste0("*", files$extension), dir(d, recursive=TRUE), value=TRUE))
	files$files = c(files$files, tmp)
}; rm(d, tmp)
...
for(file in files$files){
    cat("\r  Reading files ", (i = i + 1), "/", length(files$files), sep="")
    f <- fread(paste0(file))
    f$file = gsub(".*\\/", "", file)
    data = rbind(data, f)
}; rm(i, file, f); cat("\n")
...
summ <- data %>% group_by(tree, N, logN) %>% summarise(q05=quantile(time, 0.05),
