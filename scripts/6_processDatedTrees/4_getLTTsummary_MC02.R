...
files = list(dirsMC1="MC01-2/", # The directory where the MC01 dated trees are,
             ...
             out="MC02-2/plots/LTT_MC02_R1") # The output files without extension for the tsv table (*.tsv) and the pdf (*.pdf)
...
summ = summ[summ$tree %in% names(table(summ$tree)[table(summ$tree) > 600]),]
